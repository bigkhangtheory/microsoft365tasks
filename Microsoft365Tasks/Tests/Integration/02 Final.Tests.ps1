Describe 'Final tests' -Tags FunctionalQuality {
    ...
    It 'Every composite resource has compiled' {
        ...
        $compositeResouces = Get-DscResource -Module $env:BHProjectName
    ...
    It 'Composite resource folder count matches composite resource count' {
        ...
        $compositeResouces = Get-DscResource -Module $env:BHProjectName
        $compositeResouceFolders = Get-ChildItem -Path "$env:BHBuildOutput\Modules\$env:BHProjectName\DscResources"
